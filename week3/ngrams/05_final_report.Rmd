...
year counts : `r nrow(year_counts)`
total count: `r nrow(total_counts)`
...
=> The results look similar to the one in the paper.
...
![](ngram_view.png)
=> The graphs appear to be similar. 
...
years_and_totals %>% 
  filter(year >= 1850) %>%
  group_by(year) %>%
  summarise(total_counts = sum(total_volume)) %>%
...
=> The difference between graph (b) and graph (b) is because of the growth in number of digitized books.
In and after 1883, 1910 and 1950 these particular words were used at a high extent. The fraction
dropped however, the words rised again with the evolution of digital world and republication of the books containing those words.
...
# int_yt_df <- years_and_totals %>%
#   filter(year >= 1850) %>%
#   group_by(term) %>%
#   summarize(peak_value = max(proportion), half_of_peak_value = max(proportion)/2) %>%
#   ungroup() %>%
#   filter(peak_value == proportion) %>%
#   group_by(term)

peak_year_df <- years_and_totals %>%
  filter(year >= 1850) %>%
  group_by(term) %>%
  filter(proportion == max(proportion)) %>%
  select(term, year, proportion) %>%
  rename(peak_value = proportion, peak_year = year)
...
half_life_df <- years_and_totals %>%
  filter(year >= 1850) %>%
  group_by(term) %>%
  filter(proportion < max(proportion) * 0.5) %>%
  group_by(term) %>%
  filter(proportion == max(proportion)) %>%
  select(term, year, proportion) %>%
  rename(half_life_value = proportion, half_life_year = year)

head(half_life_df)

final_df <- left_join(peak_year_df, half_life_df, by = "term") %>% 
  mutate(diff = half_life_year - peak_year)
...
highlight_points <- final_df %>% filter(term == 1883 | term == 1910 | term == 1950)
  
final_df %>% 
  filter(term >= 1850) %>%
  ggplot(aes(x = term, y = diff)) + 
  geom_point(color = "gray") +
  geom_point(data = highlight_points, aes(x = term, y = diff), color = "red") + 
  geom_smooth(aes(x = term, y = diff)) +
...
=> Version 2 of the dataset does not produce results similar to the paper. The trend is similar. However, the results with version 2 appear to be more scattered. 
...
```{r}
sorted_by_half_life <- final_df %>% 
  filter(term >= 1850) %>%
  mutate(diff = half_life_year - peak_year) %>%
  arrange(diff)

tail(sorted_by_half_life)
head(sorted_by_half_life)


```

=> Outliers with Greatest Half Life: 
1850: Crimean War
1860 - 1863 : American Civil War
