...
print(nrow(year_counts))
print(nrow(total_counts))
...
dropped however, the volume of words rised again with the evolution of digital world and republication of the books containing those words.
...
=> It appears that the terms "1997", "1965" and "1957" have negative number of years to decline from
peak proportion to half life proportion. 
...
```{r}
#visualization of proportions for the outliers

years_and_totals %>%
  filter(term == 1997 | term == 1965 | term == 1957) %>%
  filter(year >= 1850) %>%
  mutate(term = as.factor(term)) %>%
  ggplot(aes(x = year, y = proportion, color = term)) +
  geom_line() +
